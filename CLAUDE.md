...
src/
  ...
  engine/
    ...
    touch.ts             Controles de toque réplica do Terraria mobile (joystick de movimento FIXO no canto inferior esquerdo — eixo horizontal anda, segurar pra baixo agacha, duplo-flick pra cima pula —, botão de pulo dedicado à direita, joystick de mira FIXO no canto inferior direito, tap exato/lupa de precisão fora dos controles, botões discretos de menu e fullscreen); despacho por toque pelo controle em que começou (sem "roubar" toques); inerte em dispositivos sem tela de toque
